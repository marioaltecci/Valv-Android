...
// EN: Fragment for displaying the directory structure with fixed folder loading
// RU: Фрагмент для отображения структуры папок с исправленной загрузкой
public class DirectoryFragment extends DirectoryBaseFragment {
    ...
    private PasswordViewModel passwordViewModel; // EN: Added / RU: Добавлено
    ...
    public void init() {
        ...
        // EN: Get the folder selected in PasswordFragment
        // RU: Получаем папку, выбранную в PasswordFragment
        ...
        OnBackPressedCallback callback = new OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                if (galleryViewModel.isViewpagerVisible()) {
                    ...
                    snackBarBackPressed = Snackbar.make(binding.fab, "Press back again to exit", 2000);
        ...
        if (galleryViewModel.isRootDir()) {
            ...
            if (documentFile != null && documentFile.isDirectory() && documentFile.exists()) {
                ...
                Toaster.getInstance(context).showLong("Directory does not exist");
    ...
    private void onRemoveFolderClicked(Context context) {
        Dialogs.showConfirmationDialog(context, "Remove folder",
                "Do you want to remove selected folder?",
                (dialog, which) -> {
    ...
    void addRootFolders() {
        ...
        new Thread(() -> {
            ...
            // EN: Get the dynamically selected folder from PasswordViewModel
            // RU: Получаем динамически выбранную папку из PasswordViewModel
            Uri selectedUri = passwordViewModel.getSelectedFolderUri();
            List<Uri> uriFiles = new ArrayList<>();

            if (selectedUri != null) {
                DocumentFile documentFile = DocumentFile.fromTreeUri(activity, selectedUri);
                if (documentFile != null && documentFile.canRead()) {
                    uriFiles.add(documentFile.getUri());
                }
            } else {
                // EN: Fallback to old settings ONLY if no dynamic folder is picked
                // RU: Откатываемся к старым настройкам ТОЛЬКО если папка не выбрана динамически
                List<Uri> directories = settings.getGalleryDirectoriesAsUri(true);
                for (Uri uri : directories) {
                    DocumentFile df = DocumentFile.fromTreeUri(activity, uri);
                    if (df != null && df.canRead()) uriFiles.add(df.getUri());
                }
            }
    ...
    private void addAllFolder() {
        synchronized (LOCK) {
            galleryViewModel.getGalleryFiles().add(0, GalleryFile.asAllFolder("All Files"));
            galleryGridAdapter.notifyItemInserted(0);
        }
